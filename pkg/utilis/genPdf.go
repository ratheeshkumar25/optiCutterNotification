...
func GenerateCombinedInvoicePDF(paymentID string, orderID uint, amount float64, date string, cuttingResultID uint, cuttingDetails string, components []models.ComponentPayload, filePath string) error {
	// Create a new PDF document
	pdf := gofpdf.New("P", "mm", "A4", "")

	// Page 1: Payment Invoice
	pdf.AddPage()
	...
	// Page 2: Cutting Result
	pdf.AddPage() // Add a new page for cutting result
	pdf.SetTextColor(128, 0, 128)
	pdf.SetFont("Arial", "B", 16)
	pdf.Cell(0, 10, "Opti_Cutter_Solutions")
	pdf.Ln(6)
	...
	for _, component := range components {
		...
		// Details (use MultiCell for wrapping text)
		...
		// Use MultiCell for wrapping text within the "Details" column
		pdf.SetFont("Arial", "", 10)
		pdf.MultiCell(100, 10, details, "1", "L", false)

		// Ensure the next row is placed correctly
		pdf.Ln(0)
	}

	// Footer for cutting result
	pdf.SetTextColor(0, 128, 0) // Green
	pdf.SetFont("", "", 10)
	pdf.CellFormat(90, 10, "Thank you for choosing. Welcome back again!", "", 0, "C", false, 0, "")
	pdf.Ln(12)
	...
	log.Println("Combined PDF generated successfully:", filePath)
	return nil
}
